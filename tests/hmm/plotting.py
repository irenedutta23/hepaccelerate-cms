...
def plot_variations(args):
    ...
    if((('DYLHERen' in unc) and ('dy' in mc_samp)) or (('EWZLHERen' in unc) and ('ewk_lljj_mll105_160_ptJ_herwig' in mc_samp) )):
        ...
        for k in range(len(h_nom_up.contents)):
            h_nom_up = h_nom_up*np.sum(hnom.contents)/np.sum(h_nom_up.contents)
            h_nom_down = h_nom_down*np.sum(hnom.contents)/np.sum(h_nom_down.contents)
        plot_hist_step(ax, h_nom_up.edges, h_nom_up.contents,
                np.sqrt(h_nom_up.contents_w2),
                       kwargs_step={"label": "up "+"({0:.3E})".format(np.sum(h_nom_up.contents))},
            )
...
#        dcof.write("shapes * {0} {1} $PROCESS__$CHANNEL $PROCESS__$CHANNEL__$SYSTEMATIC\n".format(
    ...
    for cat in categories:
        for proc in cat.processes:
            if proc in remove_proc:
                ...
                if("vh" not in proc and "dy_m105_160" not in proc):
                    ...
                    if("vh" in proc):
                        ...
                        if("dy_m105_160_01j" in proc):
                            ...
                            Pdf_norm_dy_m105_160_2j.append(calculate_LHEPdf_norm(dict_procs["dy_m105_160_vbf_amc_2j"], era, proc))
                            sorted(Pdf_norm_dy_m105_160_2j, reverse = True)
                            dcof.write("{0:.3f}".format(1.0 + Pdf_norm_dy_m105_160_2j[0]))
    ...
    for cat in categories:
        for proc in cat.processes:
            ...
            if "dy" in proc:
                if "160_2j" in proc:
                    QCD_norm_2j =[]
                    QCD_norm_2j.append(calculate_LHEscale_norm(dict_procs["dy_m105_160_amc_2j"], era))
                    QCD_norm_2j.append(calculate_LHEscale_norm(dict_procs["dy_m105_160_vbf_amc_2j"], era))
    ...
    for iproc in py_samp:
        dcof.write("nuisance edit rename {0} * qgl_weight qgl_weightPY_{1} \n".format(iproc,str(era))) 
    for iproc in pydipole_samp:
...
if __name__ == "__main__":
    ...
    datasets = yaml.load(open("data/datasets_NanoAODv7_Run2.yml"), Loader=yaml.FullLoader)["datasets"]
